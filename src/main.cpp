...
#if defined(NDEBUG)
# error "U2coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6b8e983ba00b7f4f993d2d5a11b7da5bec0e09b4d10bc1e8ba9f9a3feb56d16e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // U2coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "U2coin Signed Message:\n";
...
{
    // U2coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // U2coin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // U2coin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // U2coin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // U2coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // U2coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0239c7bec0b9a11de99e127555fab4b94c604972e133e6e9971800a4e3bcb966");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f8b1bf5540b3c827a0cf3697bdfbdad6093d6ef179343cc2542f9ca2daa0be250eb8af536ea2acd663e872237c244cf986c8f473ba30f769706ca0e7db248373") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1549491125;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084791788;

        if (fTestNet)
        {
            block.nTime    = 1549491094;
            block.nNonce   = 387328451;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x07cf97eae87829586d27e816a0c16068f34a774abe7d769bcd3609fddd3b9f82"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xcc, 0xb7, 0xda }; // U2coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("U2coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("U2coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static U2coinMiner(CWallet *pwallet)
{
    printf("U2coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("u2coin-miner");
    ...
    try { loop {
        ...
        printf("Running U2coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&U2coinMiner, pwallet));
}
